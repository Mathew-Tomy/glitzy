...
class _DashboardState extends State<Dashboard> {
  ...
  _getKurtaProducts() async {
    int categoryId = 18; // Use the correct category ID for Kurta
    ...
    if (response.statusCode == 200) {
      ...
      final Map<String, dynamic> responseData = json.decode(response.body);
      final List<dynamic> productLists = responseData['products'];

      if (productLists.length <= 0) {
        print('No products found in the response for Kurta');
      } else {
        setState(() {
          // Clear the existing productsKurta list
          productsKurta!.clear();

          // Parse each product and add it to the productsKurta list
          for (var productDatas in productLists) {
            productsKurta!.add(Products.fromJson(productDatas));
          }
          print(productsKurta![0].name);
        });
      }
    } else {
      print('Contact admin');
